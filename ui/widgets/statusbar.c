...
void CreateStatusBar(HWND hParent) {
  ...
  hStatusWnd = CreateWindow(szClassName, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN, 0, 0, 100,
                            STATUSBAR_HEIGHT, hParent, NULL, hInst, NULL);
                            
  hTrackbar = CreateWindowEx(0, TRACKBAR_CLASS, "",
                             WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_AUTOTICKS | TBS_BOTTOM,
                             0, 0, 140, STATUSBAR_HEIGHT - 4,
                             hStatusWnd, (HMENU)1001, hInst, NULL);
  if (hTrackbar) {
...
HWND GetStatusBarWindow(void) { return hStatusWnd; }
void StatusBarSetCoordinates(int x, int y) {
  nCurrentX = x;
  nCurrentY = y;

  if (hStatusWnd && bVisible) {
    InvalidateWindow(hStatusWnd);
  }
}
...
BOOL StatusBarIsVisible(void) { return bVisible; }
int StatusBarGetHeight(void) { return bVisible ? STATUSBAR_HEIGHT : 0; }
