...
static BOOL CrayonPreset_SaveCurrent(void) {
	...
	CrayonPresetData d = {nCrayonDensity, nCrayonTextureIntensity, nCrayonSprayAmount, nCrayonColorVariation, nCrayonBrightnessRange, nCrayonSaturationRange, nCrayonHueShiftRange, nBrushWidth};
...
static void CrayonPreset_AddNamed(const char *name, int density, int texture, int spray, int variation, int bright, int sat, int hue, int size, BOOL isDefault) {
...
void CrayonTool_RegisterPresets(void) {
	Preset_RegisterSlot(PRESET_CAT_BRUSH, PRESET_SLOT_CRAYON, CrayonPreset_Apply, CrayonPreset_SaveCurrent);
...
static float DirectionalNoise(float x, float y, float dirX, float dirY, int seed) {
	...
	if (dirLen > 0.001f) {
		...
		float perpX = -ndy;
		float perpY = ndx;
		float perpNoise = SampleNoise(x * 0.3f + perpX * 3.0f, y * 0.3f + perpY * 3.0f, s_noiseTex);
...
static void GetColorVariationNoise(float x, float y, int seed, float *brightnessNoise, float *hueNoise, float *saturationNoise) {
	...
	*brightnessNoise += GrainNoise((int)floorf(x), (int)floorf(y), seed + 32000) * 0.3f;
	if (*brightnessNoise < -1.0f)
		*brightnessNoise = -1.0f;
	if (*brightnessNoise > 1.0f)
		*brightnessNoise = 1.0f;
	...
	*saturationNoise += GrainNoise((int)floorf(x), (int)floorf(y), seed + 51000) * 0.4f;
	if (*saturationNoise < -1.0f)
		*saturationNoise = -1.0f;
	if (*saturationNoise > 1.0f)
		*saturationNoise = 1.0f;
	*saturationNoise = (*saturationNoise + 1.0f) / 2.0f;
}
static COLORREF ApplyCrayonColorVariation(COLORREF baseColor, float brightnessNoise, float hueNoise, float saturationNoise) {
	...
	float brightness = brightnessMin + brightnessNoise * (brightnessMax - brightnessMin);
...
static void CatmullRomSpline(float t, float p0x, float p0y, float p1x, float p1y, float p2x, float p2y, float p3x, float p3y, float *outX, float *outY) {
	float t2 = t * t;
	float t3 = t2 * t;
	*outX = 0.5f * ((2.0f * p1x) + (-p0x + p2x) * t + (2.0f * p0x - 5.0f * p1x + 4.0f * p2x - p3x) * t2 + (-p0x + 3.0f * p1x - 3.0f * p2x + p3x) * t3);
	*outY = 0.5f * ((2.0f * p1y) + (-p0y + p2y) * t + (2.0f * p0y - 5.0f * p1y + 4.0f * p2y - p3y) * t2 + (-p0y + 3.0f * p1y - 3.0f * p2y + p3y) * t3);
}
...
static void ApplySprayEffect(BYTE *bits, int width, int height, float centerX, float centerY, float radius, COLORREF color, BYTE colorAlpha, float pressure, int seed);
...
static void DrawCrayonSpot(BYTE *bits, int width, int height, float sx, float sy, float radius, COLORREF color, BYTE colorAlpha, float pressure, float dirX, float dirY) {
	...
	for (int py = minY; py <= maxY; py++) {
		for (int px = minX; px <= maxX; px++) {
			...
			float noise = DirectionalNoise(pixelX, pixelY, dirX, dirY, s_currentNoiseSeed);
			float brightnessNoise, hueNoise, saturationNoise;
			GetColorVariationNoise(pixelX, pixelY, s_currentNoiseSeed, &brightnessNoise, &hueNoise, &saturationNoise);
			COLORREF variedColor = ApplyCrayonColorVariation(color, brightnessNoise, hueNoise, saturationNoise);
			float textureFactor = (float)nCrayonTextureIntensity / 100.0f;
			float textureMod = noise * (0.5f + textureFactor * 1.3f);
			if (textureMod > 1.0f)
				textureMod = 1.0f;
			if (textureMod < 0.0f)
				textureMod = 0.0f;
			...
			BYTE alpha = ComposeOpacity((BYTE)(baseAlpha * pressure * textureMod * edgeFactor), colorAlpha);
			if (alpha > 0) {
				DrawPixelAlpha(bits, width, height, px, py, variedColor, alpha, LAYER_BLEND_NORMAL);
			}
		}
	}
	ApplySprayEffect(bits, width, height, sx, sy, radius, color, colorAlpha, pressure, s_currentNoiseSeed);
}
static void DrawCrayonStrokeSmooth(BYTE *bits, int width, int height, StrokePoint *p0, StrokePoint *p1, StrokePoint *p2, StrokePoint *p3, COLORREF color, BYTE colorAlpha, int size, float pressure) {
	if (!p1 || !p2)
		return;
	...
	for (int i = 0; i <= steps; i++) {
		...
		float sx, sy;
		if (p0 && p3) {
			CatmullRomSpline(t, p0->x, p0->y, p1->x, p1->y, p2->x, p2->y, p3->x, p3->y, &sx, &sy);
		} else if (p0) {
			CatmullRomSpline(t, p0->x, p0->y, p1->x, p1->y, p2->x, p2->y, p2->x, p2->y, &sx, &sy);
		} else if (p3) {
			CatmullRomSpline(t, p1->x, p1->y, p1->x, p1->y, p2->x, p2->y, p3->x, p3->y, &sx, &sy);
		} else {
			sx = p1->x + (p2->x - p1->x) * t;
			sy = p1->y + (p2->y - p1->y) * t;
		}
		DrawCrayonSpot(bits, width, height, sx, sy, radius, color, colorAlpha, pressure, dirX, dirY);
	}
}
static void ApplySprayEffect(BYTE *bits, int width, int height, float centerX, float centerY, float radius, COLORREF color, BYTE colorAlpha, float pressure, int seed) {
	...
	for (int i = 0; i < sprayCount; i++) {
		...
		if (px < 0 || px >= width || py < 0 || py >= height)
			continue;
		float brightnessNoise, hueNoise, saturationNoise;
		GetColorVariationNoise((float)px, (float)py, seed + i * 1000, &brightnessNoise, &hueNoise, &saturationNoise);
		COLORREF variedColor = ApplyCrayonColorVariation(color, brightnessNoise, hueNoise, saturationNoise);
		localSeed = (localSeed * LCG_A + LCG_C) & LCG_M;
		BYTE sprayAlpha = ComposeOpacity((BYTE)(80 + (localSeed % 100)), colorAlpha);
		DrawPixelAlpha(bits, width, height, px, py, variedColor, sprayAlpha, LAYER_BLEND_NORMAL);
	}
}
static void AddStrokePoint(int x, int y) {
	...
	if (s_strokePointCount > 0) {
		pt->pressure = CalculatePressure(&s_strokePoints[s_strokePointCount - 1], pt);
	} else {
		pt->pressure = 1.0f;
	}
	s_strokePointCount++;
}
static void DrawCrayonStroke(BYTE *bits, int width, int height, COLORREF color, BYTE colorAlpha) {
	...
	for (int i = 0; i < s_strokePointCount - 1; i++) {
		...
		StrokePoint *p3 = (i < s_strokePointCount - 2) ? &s_strokePoints[i + 2] : NULL;
		float pressure = (p1->pressure + p2->pressure) * 0.5f;
		DrawCrayonStrokeSmooth(bits, width, height, p0, p1, p2, p3, color, colorAlpha, size, pressure);
	}
}
static void CrayonTool_HandleMouseDown(HWND hWnd, int x, int y, int nButton) {
	InitNoiseTextures();
	if (!Interaction_Begin(hWnd, x, y, nButton, TOOL_CRAYON))
		return;
	...
	DrawCrayonSpot(bits, Canvas_GetWidth(), Canvas_GetHeight(), (float)x, (float)y, radius, GetColorForButton(Interaction_GetDrawButton()), colorAlpha, 1.0f, 1.0f, 0.0f);
	Interaction_MarkModified();
	Interaction_NoteStrokeSegment(x, y, x, y, GetCrayonSize() + 4);
	Interaction_FlushStrokeRedraw();
}
static void CrayonTool_HandleMouseMove(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	...
	if (bits) {
		...
		if (endIdx > startIdx) {
			...
			for (int i = startIdx; i < endIdx; i++) {
				...
				float pressure = (p1->pressure + p2->pressure) * 0.5f;
				DrawCrayonStrokeSmooth(bits, Canvas_GetWidth(), Canvas_GetHeight(), p0, p1, p2, p3, color, colorAlpha, size, pressure);
			}
			Interaction_MarkModified();
...
static void CrayonTool_HandleMouseUp(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	(void)nButton;
...
void CrayonTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		CrayonTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		CrayonTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		CrayonTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
