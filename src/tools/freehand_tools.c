...
static void PencilPt(BYTE *b, int w, int h, int x, int y, COLORREF c, BYTE a, int s) {
	(void)s;
	DrawPrim_DrawPencilPoint(b, w, h, x, y, c, a);
}
static void PencilLn(BYTE *b, int w, int h, int x1, int y1, int x2, int y2, COLORREF c, BYTE a, int s) {
	(void)s;
	DrawPrim_DrawPencilLine(b, w, h, x1, y1, x2, y2, c, a);
}
static void BrushPt(BYTE *b, int w, int h, int x, int y, COLORREF c, BYTE a, int s) {
	DrawPrim_DrawBrushPoint(b, w, h, x, y, c, a, s);
}
static void BrushLn(BYTE *b, int w, int h, int x1, int y1, int x2, int y2, COLORREF c, BYTE a, int s) {
	DrawPrim_DrawBrushLine(b, w, h, x1, y1, x2, y2, c, a, s);
}
static void EraserPt(BYTE *b, int w, int h, int x, int y, COLORREF c, BYTE a, int s) {
	(void)c;
	(void)a;
	DrawPrim_DrawEraserPoint(b, w, h, x, y, 0, s);
}
static void SprayPt(BYTE *b, int w, int h, int x, int y, COLORREF c, BYTE a, int s) {
	DrawPrim_DrawSprayPoint(b, w, h, x, y, c, a, s);
}
static int OneSize(void) {
	return 1;
}
static int BrushSize(void) {
	return nBrushWidth;
}
static int SpraySize(void) {
	return nSprayRadius;
}
static COLORREF PrimaryColor(int btn) {
	return GetColorForButton(btn);
}
static COLORREF IgnoredColor(int btn) {
	(void)btn;
	return 0;
}
static BYTE NormalAlpha(int btn) {
	return GetOpacityForButton(btn);
}
static BYTE IgnoredAlpha(int btn) {
	(void)btn;
	return 0;
}
static int PencilPad(void) {
	return 2;
}
static int BrushPad(void) {
	return DrawPrim_GetBrushSize(nBrushWidth) + 2;
}
...
static void DrawInterp(BYTE *b, int w, int h, const Policy *p, int x1, int y1, int x2, int y2, COLORREF c, BYTE a) {
	...
	if (steps == 0)
		p->pt(b, w, h, x2, y2, c, a, sz);
	else
		for (int i = 1; i <= steps; i++)
			p->pt(b, w, h, x1 + (dx * i) / steps, y1 + (dy * i) / steps, c, a, sz);
}
static void FreehandTool_HandleMouseDown(HWND hWnd, int x, int y, int btn) {
	int tool = Tool_GetCurrent();
	const Policy *p = GetPol(tool);
	if (!p || !p->pt)
		return;
	if (Interaction_IsActive()) {
		if (!Interaction_IsActiveButton(btn))
			FreehandTool_Cancel();
		return;
	}
...
static void FreehandTool_HandleMouseMove(HWND hWnd, int x, int y, int btn) {
	(void)hWnd;
	if (!Interaction_IsActive() || !Interaction_IsActiveButton(btn))
		return;
...
static void FreehandTool_HandleMouseUp(HWND hWnd, int x, int y, int btn) {
	(void)hWnd;
	(void)x;
	(void)y;
	(void)btn;
...
void FreehandTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		FreehandTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		FreehandTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		FreehandTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
...
void AirbrushTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		FreehandTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		if (Interaction_IsActive() && ev->hwnd)
			SetTimer(ev->hwnd, TIMER_AIRBRUSH, 30, NULL);
		break;
	case TOOL_POINTER_MOVE:
		FreehandTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		if (ev->hwnd)
			KillTimer(ev->hwnd, TIMER_AIRBRUSH);
		FreehandTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
