...
static void HighlighterPreset_Apply(const void *data, size_t sz) {
	if (!data || sz != sizeof(HighlighterPresetData))
		return;
...
static BOOL HighlighterPreset_SaveCurrent(void) {
	...
	HighlighterPresetData d = {nHighlighterTransparency, nHighlighterBlendMode, nHighlighterEdgeSoftness, nHighlighterOpacity, nHighlighterSizeVariation, nHighlighterTexture, nBrushWidth};
...
void HighlighterTool_RegisterPresets(void) {
	Preset_RegisterSlot(PRESET_CAT_BRUSH, PRESET_SLOT_HIGHLIGHTER, HighlighterPreset_Apply, HighlighterPreset_SaveCurrent);
#define REG(name, tr, bm, es, op, sv, tx, sz)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
	do {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
...
static int GetHighlighterSize(void) {
	...
	if (nHighlighterSizeVariation > 0) {
		...
		float variation = ((float)(variationSeed % 200) / 100.0f - 1.0f) * (nHighlighterSizeVariation / 100.0f);
...
static void HighlighterTool_HandleMouseDown(HWND hWnd, int x, int y, int nButton) {
	if (!Interaction_Begin(hWnd, x, y, nButton, TOOL_HIGHLIGHTER))
		return;
	...
	BYTE alpha = ComposeOpacity(CalcHighlighterAlpha(), GetOpacityForButton(nButton));
	{
		...
		if (r < 1)
			r = 1;
		DrawCircleAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), x, y, r, GetColorForButton(nButton), alpha, GetHighlighterBlendMode());
	}
...
static void HighlighterTool_HandleMouseMove(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	...
	if (bits) {
		...
		Interaction_GetLastPoint(&lp);
		{
			...
			if (thick < 1)
				thick = 1;
			DrawLineAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), lp.x, lp.y, x, y, thick, GetColorForButton(Interaction_GetDrawButton()), alpha, GetHighlighterBlendMode());
		}
		Interaction_MarkModified();
...
static void HighlighterTool_HandleMouseUp(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	(void)x;
	(void)y;
	(void)nButton;
...
void HighlighterTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		HighlighterTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		HighlighterTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		HighlighterTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
...
BOOL IsHighlighterDrawing(void) {
	return Interaction_IsActive() && Interaction_GetActiveToolId() == TOOL_HIGHLIGHTER;
}
void HighlighterTool_Deactivate(void) {
	if (Interaction_IsActive() && Interaction_GetActiveToolId() == TOOL_HIGHLIGHTER)
