...
static void DrawPenPoint(BYTE *bits, int width, int height, int x, int y, COLORREF color, BYTE alpha) {
	int size = GetPenSize();
	if (size <= 1) {
	...
	}
	{
		...
		if (r < 1)
			r = 1;
		DrawCircleAlpha(bits, width, height, x, y, r, color, alpha, LAYER_BLEND_NORMAL);
	}
}
static void DrawPenLine(BYTE *bits, int width, int height, int x1, int y1, int x2, int y2, COLORREF color, BYTE alpha) {
	int size = GetPenSize();
	if (size <= 1) {
		DrawLineAlpha(bits, width, height, x1, y1, x2, y2, 1, color, alpha, LAYER_BLEND_NORMAL);
		return;
	}
	DrawLineAlpha(bits, width, height, x1, y1, x2, y2, size, color, alpha, LAYER_BLEND_NORMAL);
}
static void PenTool_HandleMouseDown(HWND hWnd, int x, int y, int nButton) {
	if (!Interaction_Begin(hWnd, x, y, nButton, TOOL_PEN))
		return;
	...
	DrawPenPoint(bits, Canvas_GetWidth(), Canvas_GetHeight(), x, y, GetColorForButton(nButton), alpha);
...
static void PenTool_HandleMouseMove(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	...
	if (bits) {
		...
		DrawPenLine(bits, Canvas_GetWidth(), Canvas_GetHeight(), lp.x, lp.y, x, y, GetColorForButton(Interaction_GetDrawButton()), alpha);
		Interaction_MarkModified();
		Interaction_NoteStrokeSegment(lp.x, lp.y, x, y, GetPenSize() + 2);
	}
	Interaction_UpdateLastPoint(x, y);
}
static void PenTool_HandleMouseUp(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	(void)x;
	(void)y;
	(void)nButton;
...
void PenTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		PenTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		PenTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		PenTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
