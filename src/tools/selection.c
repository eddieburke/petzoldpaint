...
void CommitSelection(void) {
    ...
    } else { SelectionClearState(); InvalidateRect(GetCanvasWindow(), NULL, FALSE); }
}
...
void CancelSelection(void) {
    ...
    } else { SelectionClearState(); LayersClearDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE); }
}
...
void Selection_ApplySnapshot(SelectionSnapshot* s) {
    SelectionClearState();
    if (!s) { InvalidateRect(GetCanvasWindow(), NULL, FALSE); return; }
    ...
    UpdateDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE);
}
...
void SelectionFlip(BOOL horz) {
    ...
    UpdateDraft(); HistoryPushSession("Adjust Selection");
}
...
/* BlitFloatToCanvas removed (dead code) */
...
void SelectionPaste(HWND hWnd) {
    ...
    if (s_sel.hFloatBmp) {
        ...
        s_sel.rcBounds = (RECT){0, 0, w, h}; UpdateDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE);
    }
}
...
void SelectionToolOnMouseMove(HWND hWnd, int x, int y, int nButton) {
    ...
    if (s_sel.nDragMode >= HT_ROTATE_TL && s_sel.nDragMode <= HT_ROTATE_BL) {
        ...
        s_dragMoved = TRUE;
        if (Tool_GetCurrent() == TOOL_FREEFORM) {
            Poly_Add(&s_sel.freeformPts, x, y); s_sel.rcBounds = GetBoundingBox(s_sel.freeformPts.points, s_sel.freeformPts.count);
        } else {
...
void SelectionToolOnMouseUp(HWND hWnd, int x, int y, int nButton) {
    ...
    if (s_modeAtDragStart == SEL_NONE && !s_dragMoved) { SelectionClearState(); LayersClearDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE); }
    else if (Tool_GetCurrent() == TOOL_FREEFORM && s_sel.freeformPts.count > 2) s_sel.hRegion = Poly_CreateRegion(&s_sel.freeformPts);
    if (s_dragMoved && s_sel.mode != SEL_NONE) HistoryPushSession(s_modeAtDragStart == SEL_NONE ? "Create Selection" : "Adjust Selection");
    s_sel.nDragMode = HT_NONE; s_dragMoved = FALSE;
}

void SelectionTool_OnCaptureLost(void) {
    s_sel.nDragMode = HT_NONE; s_dragMoved = FALSE;
    if (s_modeAtDragStart == SEL_NONE && s_sel.mode == SEL_REGION_ONLY) { SelectionClearState(); LayersClearDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE); }
}
...
void SelectionSelectAll(void) {
    ...
    s_sel.mode = SEL_REGION_ONLY; InvalidateRect(GetCanvasWindow(), NULL, FALSE);
}

void SelectionMove(int dx, int dy) {
    if (s_sel.mode != SEL_FLOATING) LiftSelectionPixels();
    if (s_sel.mode == SEL_FLOATING) { OffsetRect(&s_sel.rcBounds, dx, dy); UpdateDraft(); InvalidateRect(GetCanvasWindow(), NULL, FALSE);
        HistoryPushSession("Adjust Selection"); }
}
