...
#include <stdlib.h>
typedef enum { STATE_IDLE = 0, STATE_DRAWING_LINE, STATE_WAIT_CTRL1, STATE_DRAG_CTRL1, STATE_WAIT_CTRL2, STATE_DRAG_CTRL2, STATE_EDITING, STATE_DRAG_EDIT_HANDLE } BezierState;
...
static void DrawBezierSegment(BYTE *bits, int width, int height, COLORREF color, BYTE alpha, int thickness) {
	POINT
	p0 = ptStart, p1 = ptCtrl1, p2 = ptCtrl2, p3 = ptEnd;
	int steps = 100;
	...
	for (int i = 1; i <= steps; i++) {
		...
		DrawLineStampCircles(bits, width, height, prevX, prevY, x, y, radius, color, alpha, LAYER_BLEND_NORMAL);
		prevX = x;
		prevY = y;
	}
}
...
static void BezierTool_HandleMouseDown(HWND hWnd, int x, int y, int nButton) {
	...
	switch (state) {
	case STATE_IDLE:
		if (!Interaction_BeginEx(hWnd, x, y, nButton, TOOL_CURVE, FALSE))
			break;
		nDrawButton = nButton;
		ptStart.x = x;
		ptStart.y = y;
	...
	case STATE_EDITING: {
		...
		COMMIT_BAR_HANDLE_CLICK(&rcBounds, x, y, BezierCommitCurve(), BezierTool_Cancel());
...
BOOL BezierTool_HandleOverlayClick(HWND hWnd, int screenX, int screenY, int nButton) {
	(void)hWnd;
	if (state != STATE_EDITING || nButton != MK_LBUTTON)
		return FALSE;
	RECT rcBounds;
	BezierGetBounds(&rcBounds);
...
static void BezierTool_HandleMouseMove(HWND hWnd, int x, int y, int nButton) {
	(void)nButton;
	if (GetCapture() != hWnd)
		return;
	switch (state) {
...
static void BezierTool_HandleMouseUp(HWND hWnd, int x, int y, int nButton) {
	(void)nButton;
	if (GetCapture() != hWnd)
		return;
...
void BezierTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		BezierTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		BezierTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		BezierTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
...
BezierToolSnapshot *BezierTool_CreateSnapshot(void) {
	if (state == STATE_IDLE)
		return NULL;
	BezierToolSnapshot *snapshot = (BezierToolSnapshot *)calloc(1, sizeof(BezierToolSnapshot));
	if (!snapshot)
		return NULL;
...
void BezierTool_ApplySnapshot(const BezierToolSnapshot *snapshot) {
	...
	if (state == STATE_DRAG_CTRL1 || state == STATE_DRAG_CTRL2 || state == STATE_DRAG_EDIT_HANDLE)
		state = STATE_EDITING;
	...
	nEditHandle = -1;
	if (!Interaction_IsActive()) {
		(void)Interaction_BeginEx(GetCanvasWindow(), ptStart.x, ptStart.y, nDrawButton, TOOL_CURVE, FALSE);
	}
	UpdateDraftLayer();
	InvalidateRect(GetCanvasWindow(), NULL, FALSE);
}
