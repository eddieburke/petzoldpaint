...
static void FillPolygonAlpha(BYTE *bits, int width, int height, COLORREF color, BYTE alpha) {
	if (polygon.count < 3)
		return;
	...
	for (int y = minY; y <= maxY; y++) {
		...
		for (int i = 0; i < polygon.count && numIntersections < maxIntersections - 2; i++) {
			...
			if ((y1 <= y && y2 > y) || (y2 <= y && y1 > y)) {
				...
				intersections[numIntersections++] = x1 + (y - y1) * (x2 - x1) / (y2 - y1);
			}
		}
...
static void DrawPolygonToDraft(void) {
	...
	for (int i = 0; i < polygon.count - 1; i++) {
		DrawLineAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), polygon.points[i].x, polygon.points[i].y, polygon.points[i + 1].x, polygon.points[i + 1].y, lineW, fg, fgAlpha, LAYER_BLEND_NORMAL);
	}
	if (polygon.count > 0) {
		POINT last = polygon.points[polygon.count - 1];
		DrawLineAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), last.x, last.y, ptRubberBand.x, ptRubberBand.y, lineW, fg, (BYTE)(fgAlpha / 2), LAYER_BLEND_NORMAL);
	}
	LayersMarkDirty();
}
static void CommitPolygonInternal(void) {
	...
	if (bits) {
		...
		if ((nShapeDrawType == 1 || nShapeDrawType == 2) && polygon.count >= 3) {
			FillPolygonAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), (nShapeDrawType == 1) ? fg : bg, (nShapeDrawType == 1) ? fgAlpha : bgAlpha);
		}
		if (nShapeDrawType == 0 || nShapeDrawType == 2) {
			for (int i = 0; i < polygon.count; i++) {
				int next = (i + 1) % polygon.count;
				DrawLineAlpha(bits, Canvas_GetWidth(), Canvas_GetHeight(), polygon.points[i].x, polygon.points[i].y, polygon.points[next].x, polygon.points[next].y, lineW, fg, fgAlpha, LAYER_BLEND_NORMAL);
			}
		}
	}
...
static void StartNewPolygon(HWND hWnd, int x, int y, int nButton) {
	nDrawButton = nButton;
	Poly_Init(&polygon);
	if (!Interaction_Begin(hWnd, x, y, nButton, TOOL_POLYGON)) {
		Poly_Free(&polygon);
		return;
	}
	if (!Poly_Add(&polygon, x, y)) {
...
static void PolygonTool_HandleMouseDown(HWND hWnd, int x, int y, int nButton) {
...
BOOL PolygonTool_HandleOverlayClick(HWND hWnd, int screenX, int screenY, int nButton) {
	(void)hWnd;
	if (!bPolygonPending || polygon.count <= 0 || nButton != MK_LBUTTON)
		return FALSE;
	RECT rcBounds = GetBoundingBox(polygon.points, polygon.count);
...
static void PolygonTool_HandleMouseMove(HWND hWnd, int x, int y, int nButton) {
	(void)hWnd;
	(void)nButton;
	if (!bPolygonPending)
		return;
...
static void PolygonTool_HandleMouseUp(HWND hWnd, int x, int y, int nButton) {
	(void)nButton;
	if (GetCapture() == hWnd) {
...
static void PolygonTool_HandleDoubleClick(HWND hWnd, int x, int y, int nButton) {
	(void)nButton;
	if (bPolygonPending) {
		if (!PolygonTryAddPoint(hWnd, x, y))
			return;
...
void PolygonTool_OnPointer(const ToolPointerEvent *ev) {
	...
	case TOOL_POINTER_DOWN:
		PolygonTool_HandleMouseDown(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_MOVE:
		PolygonTool_HandleMouseMove(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_UP:
		PolygonTool_HandleMouseUp(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	case TOOL_POINTER_DOUBLE_CLICK:
		PolygonTool_HandleDoubleClick(ev->hwnd, ev->bmp.x, ev->bmp.y, (int)ev->buttons);
		break;
	default:
		break;
	}
...
void PolygonTool_DrawOverlay(const OverlayContext *ctx) {
	...
	if (nDragIndex < 0 && polygon.count > 0) {
		POINT last = Poly_GetLast(&polygon);
		Overlay_DrawLine(ctx, last.x, last.y, ptRubberBand.x, ptRubberBand.y, fg, PS_SOLID);
		if (polygon.count >= 2) {
			Overlay_DrawLine(ctx, ptRubberBand.x, ptRubberBand.y, polygon.points[0].x, polygon.points[0].y, RGB(128, 128, 128), PS_DOT);
		}
	}
...
PolygonToolSnapshot *PolygonTool_CreateSnapshot(void) {
	if (!bPolygonPending)
		return NULL;
	PolygonToolSnapshot *snapshot = (PolygonToolSnapshot *)calloc(1, sizeof(PolygonToolSnapshot));
	if (!snapshot)
		return NULL;
...
void PolygonTool_ApplySnapshot(const PolygonToolSnapshot *snapshot) {
	...
	if (bPolygonPending && polygon.count > 0 && !Interaction_IsActive()) {
		...
		(void)Interaction_BeginEx(GetCanvasWindow(), p0.x, p0.y, nDrawButton, TOOL_POLYGON, FALSE);
	}
	DrawPolygonToDraft();
	InvalidateRect(GetCanvasWindow(), NULL, FALSE);
}
