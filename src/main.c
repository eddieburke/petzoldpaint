...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR szCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowW(szAppName, L"Peztold Paint", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);
	if (!hwnd) {
		return 0;
	}
	hMainWnd = hwnd;
...
void ResizeLayout(HWND hwnd) {
	...
	if (hStatus)
		MoveWindow(hStatus, 0, rc.bottom - nStatusHeight, rc.right, nStatusHeight, TRUE);
	nBottomOffset = nStatusHeight + COLORBOX_HEIGHT;
	int nToolbarButtonsHeight = GetToolbarHeight();
	HWND hToolbar = GetToolbarWindow();
	if (hToolbar)
		MoveWindow(hToolbar, 0, 0, TOOLBAR_WIDTH, nToolbarButtonsHeight, TRUE);
	HWND hToolOptions = GetToolOptionsWindow();
	if (hToolOptions)
		MoveWindow(hToolOptions, 0, nToolbarButtonsHeight, TOOLBAR_WIDTH, rc.bottom - nBottomOffset - nToolbarButtonsHeight, TRUE);
	HWND hColorbox = GetColorboxWindow();
	if (hColorbox)
		MoveWindow(hColorbox, 0, rc.bottom - nBottomOffset, rc.right, COLORBOX_HEIGHT, TRUE);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CONTEXTMENU: {
		...
		if (curTool == TOOL_SELECT || curTool == TOOL_FREEFORM || curTool == TOOL_TEXT) {
			HMENU hMenu = LoadMenu(hInst, MAKEINTRESOURCE(IDR_CONTEXT_MENU));
			HMENU hSub = GetSubMenu(hMenu, 0);
			TrackPopupMenu(hSub, TPM_RIGHTBUTTON, screenX, screenY, 0, hwnd, NULL);
			DestroyMenu(hMenu);
			return 0;
		}
	}
