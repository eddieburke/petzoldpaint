...
static void RefreshHistoryList(void) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (hdc) {
			SIZE sz;
			if (GetTextExtentPoint32(hdc, display, lstrlen(display), &sz) && sz.cx > maxTextPx) {
				maxTextPx = sz.cx;
			}
		}
	}
...
static void LayoutControls(HWND hwnd) {
	...
	if (listHeight < 70)
		listHeight = 70;
	MoveWindow(hHistoryList, pad + groupPad / 2, y + groupTitle, width - pad * 2 - groupPad, listHeight, TRUE);
}
...
static LRESULT CALLBACK HistoryPanelWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
	case WM_CREATE: {
		...
		struct {
			...
			style;
			int id;
		} controls[] = {{&hGroupHistory, 0, "BUTTON", historyGroupTitle, WS_CHILD | WS_VISIBLE | BS_GROUPBOX, IDC_GROUP_HISTORY}, {&hHistoryList, WS_EX_CLIENTEDGE, "LISTBOX", "", WS_CHILD | WS_VISIBLE | LBS_NOTIFY | WS_VSCROLL | WS_HSCROLL | LBS_NOINTEGRALHEIGHT, IDC_HISTORY_LIST}};
		StringCchPrintf(historyGroupTitle, sizeof(historyGroupTitle), "History (Last %d)", HISTORY_MAX_ENTRIES);
...
void CreateHistoryPanel(HWND hParent) {
	...
	RegisterClass(&wc);
	hHistoryWnd = CreateWindowEx(WS_EX_TOOLWINDOW, szClassName, "History", WS_POPUP | WS_CAPTION | WS_THICKFRAME | WS_SYSMENU, 400, 120, 280, 450, hParent, NULL, hInst, NULL);
	if (hHistoryWnd) {
...
void HistoryPanelSync(void) {
	if (!hHistoryWnd)
		return;
	RefreshHistoryList();
	InvalidateRect(hHistoryWnd, NULL, FALSE);
}
