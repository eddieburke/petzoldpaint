...
static void UpdateControlsFromLayer(void) {
	...
	if (idx < 0)
		return;
	SendMessage(hChkVisible, BM_SETCHECK, LayersGetVisible(idx) ? BST_CHECKED : BST_UNCHECKED, 0);
...
static void LayoutControls(HWND hwnd) {
	...
	if (listHeight < 70)
		listHeight = 70;
	MoveWindow(hLayerList, pad + groupPad / 2, y + groupTitle, width - pad * 2 - groupPad, listHeight, TRUE);
...
static void HandleVisibleToggle(void) {
	...
	if (idx < 0)
		return;
	BOOL newVisible = (SendMessage(hChkVisible, BM_GETCHECK, 0, 0) == BST_CHECKED);
...
static LRESULT CALLBACK LayersPanelWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
	case WM_CREATE: {
		struct {
			HWND *hwnd;
			...
			style;
			int id;
		} controls[] = {{&hGroupLayers, 0, "BUTTON", "Layers", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, IDC_GROUP_LAYERS},
		                {&hGroupProps, 0, "BUTTON", "Layer Options", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, IDC_GROUP_PROPS},
		                {&hLayerList, WS_EX_CLIENTEDGE, "LISTBOX", "", WS_CHILD | WS_VISIBLE | LBS_NOTIFY | WS_VSCROLL, IDC_LAYER_LIST},
		                {&hBtnAdd, 0, "BUTTON", "Add", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, IDC_BTN_ADD},
		                {&hBtnDelete, 0, "BUTTON", "Del", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, IDC_BTN_DELETE},
		                {&hBtnUp, 0, "BUTTON", "Up", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, IDC_BTN_UP},
		                {&hBtnDown, 0, "BUTTON", "Down", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, IDC_BTN_DOWN},
		                {&hBtnMerge, 0, "BUTTON", "Merge Down", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, IDC_BTN_MERGE},
		                {&hChkVisible, 0, "BUTTON", "Visible", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, IDC_CHK_VISIBLE},
		                {&hOpacityLabel, 0, "STATIC", "Opacity: 100%", WS_CHILD | WS_VISIBLE, IDC_OPACITY_LABEL},
		                {&hOpacity, 0, TRACKBAR_CLASS, "", WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS, IDC_OPACITY_SLIDER},
		                {&hBlend, 0, "COMBOBOX", "", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, IDC_BLEND_COMBO}};
		hUiFont = CreateSegoiUIFont(-11, FW_NORMAL);
		for (int i = 0; i < (int)(sizeof(controls) / sizeof(controls[0])); i++) {
			*controls[i].hwnd = CreateWindowEx(controls[i].exStyle, controls[i].cls, controls[i].text, controls[i].style, 0, 0, 10, 10, hwnd, (HMENU)(INT_PTR)controls[i].id, hInst, NULL);
			if (hUiFont) {
		...
		for (int i = 0; i < (int)(sizeof(blendModes) / sizeof(blendModes[0])); i++) {
	...
	case WM_HSCROLL:
		if ((HWND)lParam == hOpacity) {
			...
			BOOL bEndTrack = (LOWORD(wParam) == TB_ENDTRACK) || (LOWORD(wParam) == TB_THUMBPOSITION);
			if (bEndTrack) {
...
void CreateLayersPanel(HWND hParent) {
	...
	RegisterClass(&wc);
	hLayersWnd = CreateWindowEx(WS_EX_TOOLWINDOW, szClassName, NULL, WS_POPUP | WS_CAPTION | WS_THICKFRAME | WS_SYSMENU, 120, 120, 250, 400, hParent, NULL, hInst, NULL);
	if (hLayersWnd) {
		ShowWindow(hLayersWnd, SW_SHOWNOACTIVATE);
	}
}
HWND GetLayersPanelWindow(void) {
	return hLayersWnd;
}
void LayersPanelSync(void) {
	if (!hLayersWnd)
		return;
	RefreshLayerList();
	InvalidateRect(hLayersWnd, NULL, FALSE);
}
