...
LRESULT CALLBACK ToolbarWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_DESTROY:
		if (hToolIcons) {
			DeleteObject(hToolIcons);
			hToolIcons = NULL;
		}
		if (hToolIconsExtra) {
			DeleteObject(hToolIconsExtra);
			hToolIconsExtra = NULL;
		}
		return 0;
	}
...
void CreateToolbar(HWND hParent) {
	...
	hToolbarWnd = CreateWindow(szAppName, NULL, WS_CHILD | WS_VISIBLE, 0, 0, TOOLBAR_WIDTH, 400, hParent, NULL, hInst, NULL);
}
	...
	for (int i = 0; i < NUM_TOOLS; i++) {
		...
		if (hToolIcons || hToolIconsExtra) {
			...
			if (hActiveBmp) {
				...
				TransparentBlt(hdc, iconX, iconY, ICON_WIDTH, ICON_HEIGHT, hdcMem, srcX, 0, ICON_WIDTH, ICON_HEIGHT, ICON_COLORKEY);
			}
		}
	}
	if (hOldBmp)
