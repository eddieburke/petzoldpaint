...
static BOOL HandleZoomMenu(HWND hwnd, WORD id) {
	static const ZoomMenuItem kZoomItems[] = {{IDM_ZOOM_100, 100.0}, {IDM_ZOOM_400, 400.0}};
...
static BOOL CanPasteFromClipboard(void) {
	...
	if (uPng && IsClipboardFormatAvailable(uPng))
		return TRUE;
	return IsClipboardFormatAvailable(CF_DIBV5) || IsClipboardFormatAvailable(CF_DIB) || IsClipboardFormatAvailable(CF_BITMAP);
}
...
void DocumentOpen(HWND hwnd, const wchar_t *path) {
	...
	if (path) {
		...
		if (loaded) {
			...
			MessageBoxW(hwnd, L"Could not open the file.", L"Open", MB_OK | MB_ICONERROR);
		}
	} else {
...
void AppCommands_OnInitMenuPopup(HWND hwnd, WPARAM wParam, LPARAM lParam) {
	...
	CheckMenuItem(hMenu, IDM_SHOW_COMMIT_BAR, MF_BYCOMMAND | (CommitBar_IsEnabled() ? MF_CHECKED : MF_UNCHECKED));
}
