...
static void HandleScrollEx(HWND hwnd, int nBar, int nScrollCode, int lineDelta) {
  ...
  int nPos;
  int *pScrollPos =
      (nBar == SB_HORZ) ? Canvas_GetScrollXPtr() : Canvas_GetScrollYPtr();
...
void Controller_HandleMouseDown(HWND hwnd, int screenX, int screenY, int btn) {
  ...
  int xBitmap, yBitmap;
  if (!BmpCoordInBounds(screenX, screenY, &xBitmap, &yBitmap)) return;
...
void Controller_HandleMouseUp(HWND hwnd, int screenX, int screenY, int btn) {
  if (s_resize.active) {
    ...
    if (s_resize.newW != Canvas_GetWidth() || s_resize.newH != Canvas_GetHeight()) {
      char desc[64];
      StringCchPrintf(desc, sizeof(desc), "Resize Canvas: %dx%d -> %dx%d",
                      Canvas_GetWidth(), Canvas_GetHeight(), s_resize.newW,
                      s_resize.newH);
