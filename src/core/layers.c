...
static void ClearLayer(Layer *l) {
	if (l->bmp) {
		DeleteObject(l->bmp);
		l->bmp = NULL;
		l->bits = NULL;
...
static HBITMAP CreateLayerBmp(int ww, int hh, BYTE **bits) {
	...
	void *ptr;
	HBITMAP h = CreateDIBSection(NULL, &bi, DIB_RGB_COLORS, &ptr, NULL, 0);
	*bits = (BYTE *)ptr;
	return h;
}
...
BOOL LayersResize(int ww, int hh) {
	...
	for (int i = 0; i < lc; i++) {
		...
		for (int y = 0; y < ch; y++)
			memcpy(newBits[i] + (size_t)y * ww * 4, oldBits[i] + (size_t)y * ow * 4, (size_t)cw * 4);
...
BOOL LayersMergeDown(int i) {
	...
	for (int y = 0; y < hh; y++)
		for (int x = 0; x < ww; x++) {
			...
			if (!s->visible || s->opacity == 0 || sp[3] == 0)
				continue;
			int a = sp[3] * s->opacity / 255;
			if (a <= 0)
				continue;
...
BOOL LayersGetVisible(int i) {
	return i >= 0 && i < lc ? lyr[i].visible : FALSE;
}
BOOL LayersSetVisible(int i, BOOL v) {
	if (i < 0 || i >= lc || lyr[i].visible == v)
		return FALSE;
	lyr[i].visible = v;
	MarkFullDirty();
	return TRUE;
...
BYTE LayersGetOpacity(int i) {
	return i >= 0 && i < lc ? lyr[i].opacity : 255;
}
BOOL LayersSetOpacity(int i, BYTE o) {
	if (i < 0 || i >= lc || lyr[i].opacity == o)
		return FALSE;
	lyr[i].opacity = o;
	MarkFullDirty();
	return TRUE;
...
int LayersGetBlendMode(int i) {
	return i >= 0 && i < lc ? lyr[i].blend : 0;
}
BOOL LayersSetBlendMode(int i, int b) {
	if (i < 0 || i >= lc || lyr[i].blend == b)
		return FALSE;
	lyr[i].blend = b;
	MarkFullDirty();
	return TRUE;
...
void LayersGetName(int i, char *out, int sz) {
	if (i >= 0 && i < lc && out && sz > 0)
		strncpy_s(out, sz, lyr[i].name, _TRUNCATE);
}
...
HBITMAP
LayersGetActiveColorBitmap(void) {
	return lc > 0 && act >= 0 && act < lc ? lyr[act].bmp : NULL;
}
BYTE *LayersGetActiveColorBits(void) {
	return lc > 0 && act >= 0 && act < lc ? lyr[act].bits : NULL;
}
BYTE *Layers_BeginWrite(void) {
	return LayersGetActiveColorBits();
}
static void CompositeFillBackground(BYTE *dst, int ww, int hh, int x0, int y0, int x1, int y1, BOOL check, COLORREF bg) {
	if (check)
		PixelOps_FillCheckerboardRect(dst, ww, hh, x0, y0, x1, y1);
	else
		PixelOps_FillRect(dst, ww, hh, bg, 255, x0, y0, x1, y1);
}
static void CompositePixelAt(BYTE *dst, int ww, int hh, int x, int y, BOOL blendDraft) {
...
static void CompositeRegionSingleLayer(BYTE *dst, int ww, int hh, int x0, int y0, int x1, int y1, const Layer *layer, BOOL blendDraft) {
	...
	for (y = y0; y < y1; y++) {
		...
		BYTE *fp = useDraft ? (draftBits + ((size_t)y * (size_t)ww + (size_t)x0) * 4) : NULL;
		for (x = x0; x < x1; x++) {
			BYTE sa = sp[3];
			if (sa == 255) {
				dp[0] = sp[0];
			...
			if (fp && fp[3]) {
				PixelOps_BlendPixel(fp[2], fp[1], fp[0], fp[3], dp, LAYER_BLEND_NORMAL);
			}
			dp += 4;
			sp += 4;
			if (fp)
...
static void CompositeRegion(BYTE *dst, int ww, int hh, int x0, int y0, int x1, int y1, BOOL check, COLORREF bg) {
...
LayersGetCompositeBitmap(void) {
	...
	if (dirty) {
		if (fullCompositeDirty || !dirtyRectValid || !compBits) {
			...
			CompositeRegion(compBits, ww, hh, dirtyRect.left, dirtyRect.top, dirtyRect.right, dirtyRect.bottom, check, bg);
		}
		dirty = FALSE;
...
HBITMAP
LayersFlattenToBitmap(COLORREF bg) {
	int ww = Canvas_GetWidth(), hh = Canvas_GetHeight();
	BYTE *bits;
	HBITMAP bmp = CreateLayerBmp(ww, hh, &bits);
	if (bmp)
		Composite(bits, ww, hh, FALSE, bg);
	return bmp;
}
HBITMAP
LayersFlattenToBitmapWithAlpha(BYTE **out) {
			...
			if (lyr[i].visible && lyr[i].opacity)
				for (int y = 0; y < hh; y++)
					for (int x = 0; x < ww; x++) {
						...
						if (sp[3])
							PixelOps_BlendPixel(sp[2], sp[1], sp[0], sp[3] * lyr[i].opacity / 255, dp, 0);
					}
		if (out)
			*out = bits;
	}
...
COLORREF
LayersSampleCompositeColor(int x, int y) {
	int ww = Canvas_GetWidth(), hh = Canvas_GetHeight();
...
LayerSnapshot *LayersCreateSnapshot(void) {
	...
	for (int i = 0; i < lc; i++) {
		...
		memcpy(s->layers[i].colorBits, lyr[i].bits, (size_t)s->width * (size_t)s->height * 4);
		s->layers[i].opacity = lyr[i].opacity;
		s->layers[i].blendMode = lyr[i].blend;
		s->layers[i].visible = lyr[i].visible;
		strncpy_s(s->layers[i].name, sizeof(s->layers[i].name), lyr[i].name, _TRUNCATE);
	}
	return s;
}
BOOL LayersApplySnapshot(LayerSnapshot *s) {
	...
	for (int i = 0; i < lc; i++) {
		...
		memcpy(lyr[i].bits, s->layers[i].colorBits, (size_t)s->width * (size_t)s->height * 4);
...
void LayersDestroySnapshot(LayerSnapshot *s) {
	...
	for (int i = 0; i < s->layerCount; i++)
		if (s->layers[i].colorBmp)
			DeleteObject(s->layers[i].colorBmp);
	free(s);
}
...
BOOL LayersLoadFromPixels(int ww, int hh, const BYTE *bgra, int stride, BOOL useAlpha) {
	...
	for (int y = 0; y < hh; y++) {
		const BYTE *sr = bgra + y * stride;
		BYTE *ds = lyr[0].bits + y * ww * 4;
		for (int x = 0; x < ww; x++) {
			ds[x * 4] = sr[x * 4];
			ds[x * 4 + 1] = sr[x * 4 + 1];
			ds[x * 4 + 2] = sr[x * 4 + 2];
			ds[x * 4 + 3] = useAlpha ? sr[x * 4 + 3] : 255;
		}
	}
	lc = 1;
	act = 0;
...
void LayersEnsureDraft(void) {
	...
	draft = CreateLayerBmp(ww, hh, &draftBits);
	draftW = ww;
	draftH = hh;
	if (draftBits)
...
void LayersMergeDraftRect(const RECT *bmpBounds) {
	...
	if (!draftBits || draftW != Canvas_GetWidth() || draftH != Canvas_GetHeight())
		return;
