...
void Canvas_SetWidth(int w) {
	Doc_SetSize(w, Doc_GetHeight());
}
int Canvas_GetHeight(void) {
	return Doc_GetHeight();
}
void Canvas_SetHeight(int h) {
	Doc_SetSize(Doc_GetWidth(), h);
}
int Canvas_GetScrollX(void) {
	return Doc_GetScrollX();
}
...
int *Canvas_GetScrollXPtr(void) {
	return &GetDocument()->scrollX;
}
int Canvas_GetScrollY(void) {
	return Doc_GetScrollY();
}
...
static void DrawResizeHandles(HDC hdc, int nDestX, int nDestY, int nScaledW, int nScaledH);
LRESULT CALLBACK CanvasWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);
BOOL Canvas_IsValidSize(int width, int height) {
	return width >= CANVAS_MIN_DIM && height >= CANVAS_MIN_DIM && width <= CANVAS_MAX_DIM && height <= CANVAS_MAX_DIM;
}
BOOL CreateCanvas(int width, int height) {
	if (!Canvas_IsValidSize(width, height))
		return FALSE;
...
BOOL ResizeCanvas(int nNewWidth, int nNewHeight) {
	if (!Canvas_IsValidSize(nNewWidth, nNewHeight))
		return FALSE;
...
static void DrawResizeHandles(HDC hdc, int nDestX, int nDestY, int nScaledW, int nScaledH) {
	...
	Rectangle(hdc, nDestX + nScaledW + 2, nDestY + nScaledH + 2, nDestX + nScaledW + 2 + HANDLE_SIZE, nDestY + nScaledH + 2 + HANDLE_SIZE);
	Rectangle(hdc, nDestX + nScaledW + 2, nDestY + nScaledH / 2 - HANDLE_SIZE / 2, nDestX + nScaledW + 2 + HANDLE_SIZE, nDestY + nScaledH / 2 + HANDLE_SIZE / 2);
	Rectangle(hdc, nDestX + nScaledW / 2 - HANDLE_SIZE / 2, nDestY + nScaledH + 2, nDestX + nScaledW / 2 + HANDLE_SIZE / 2, nDestY + nScaledH + 2 + HANDLE_SIZE);
...
LRESULT CALLBACK CanvasWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT: {
		...
		RECT rcBorder = {nDestX - 1, nDestY - 1, nDestX + nScaledW + 1, nDestY + nScaledH + 1};
		...
		if (hComposite) {
			...
			if (hBmpDC) {
				...
				StretchBlt(s_hViewDC, nDestX, nDestY, nScaledW, nScaledH, hBmpDC, 0, 0, Canvas_GetWidth(), Canvas_GetHeight(), SRCCOPY);
				SelectObject(hBmpDC, hOldBmp);
				DeleteDC(hBmpDC);
			}
		}
		OverlayContext overlayCtx;
		Overlay_Init(&overlayCtx, s_hViewDC, dScale, nDestX, nDestY);
		ToolDrawOverlay(&overlayCtx);
		...
		if (Controller_IsResizing()) {
			...
			if (prevW != Canvas_GetWidth() || prevH != Canvas_GetHeight()) {
				...
				HPEN hOldPen;
				HPEN hPenDotted = CreatePenAndSelect(s_hViewDC, PS_DOT, 1, RGB(0, 0, 0), &hOldPen);
				HBRUSH hOldBrush2 = (HBRUSH)SelectObject(s_hViewDC, GetStockObject(NULL_BRUSH));
				Rectangle(s_hViewDC, nDestX, nDestY, nDestX + nPreviewW, nDestY + nPreviewH);
				RestorePen(s_hViewDC, hOldPen);
				SelectObject(s_hViewDC, hOldBrush2);
				Gdi_DeletePen(hPenDotted);
			}
		}
		BitBlt(hdc, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left, ps.rcPaint.bottom - ps.rcPaint.top, s_hViewDC, ps.rcPaint.left, ps.rcPaint.top, SRCCOPY);
		EndPaint(hwnd, &ps);
	}
		return 0;
	case WM_ERASEBKGND:
	...
	case WM_SETCURSOR:
		...
		break;
	case WM_LBUTTONDOWN:
		Controller_HandleMouseDown(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), MK_LBUTTON);
		return 0;
	case WM_LBUTTONDBLCLK:
		Controller_HandleDoubleClick(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), MK_LBUTTON);
		return 0;
	case WM_RBUTTONDOWN:
		Controller_HandleMouseDown(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), MK_RBUTTON);
		return 0;
	case WM_MOUSEMOVE:
		Controller_HandleMouseMove(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), (int)wParam);
		return 0;
	case WM_LBUTTONUP:
		Controller_HandleMouseUp(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), MK_LBUTTON);
		return 0;
	case WM_RBUTTONUP:
		Controller_HandleMouseUp(hwnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), MK_RBUTTON);
		return 0;
...
void CreateCanvasWindow(HWND hParent) {
	...
	RegisterClass(&wc);
	hCanvasWnd = CreateWindow(szClassName, NULL, WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL, 0, 0, 100, 100, hParent, NULL, hInst, NULL);
}
