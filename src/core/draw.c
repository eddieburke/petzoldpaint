...
void ResizeRect(RECT *rc, int handle, int dx, int dy, int minSize, BOOL bKeepAspect) {
	...
	if (bKeepAspect && (handle == HT_TL || handle == HT_TR || handle == HT_BR || handle == HT_BL)) {
		if (origW != 0 && origH != 0) {
...
static void DrawLineAlphaThickVisit(BYTE *bits, int width, int height, int x, int y, void *userData) {
...
void DrawLineAlpha(BYTE *bits, int width, int height, int x1, int y1, int x2, int y2, int thickness, COLORREF color, BYTE alpha, int mode) {
	...
	ctx.alpha = alpha;
	ctx.mode = mode;
	DrawLineSpineEach(bits, width, height, x1, y1, x2, y2, DrawLineAlphaThickVisit, &ctx);
}
...
void DrawLineStampCircles(BYTE *bits, int width, int height, float x1, float y1, float x2, float y2, float radius, COLORREF color, BYTE alpha, int mode) {
	...
	if (dx * dx + dy * dy < 0.01f) {
		...
		if (ir < 1)
			ir = 1;
		DrawCircleAlpha(bits, width, height, (int)x1, (int)y1, ir, color, alpha, mode);
		return;
	}
	int thick = (int)ceilf(2.0f * radius);
	if (thick < 1)
		thick = 1;
	DrawLineAlpha(bits, width, height, (int)x1, (int)y1, (int)x2, (int)y2, thick, color, alpha, mode);
}
...
void DrawRectOutlineAlpha(BYTE *bits, int width, int height, int x, int y, int w, int h, COLORREF color, BYTE alpha, int thickness, int mode) {
	...
	DrawRectAlpha(bits, width, height, x, y + h - thickness, w, thickness, color, alpha, mode);
	DrawRectAlpha(bits, width, height, x, y + thickness, thickness, h - 2 * thickness, color, alpha, mode);
	DrawRectAlpha(bits, width, height, x + w - thickness, y + thickness, thickness, h - 2 * thickness, color, alpha, mode);
}
...
static float sd_rounded_rect(float px, float py, float half_w, float half_h, float rad) {
	if (rad <= 0.f) {
