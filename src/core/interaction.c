...
static void StrokeDirtyReset(void) { s_strokeDirtyValid = FALSE; }
...
void Interaction_FlushStrokeRedraw(void) {
  if (!s_strokeDirtyValid)
    return;
  LayersMarkDirtyRect(s_strokeDirty.left, s_strokeDirty.top,
                      s_strokeDirty.right, s_strokeDirty.bottom);
...
static BOOL FinalizeInteraction(const char *pushHistory) {
  ...
  if (changed) {
    if (pushHistory) HistoryPush(pushHistory);
