...
static struct { BOOL active, modified, capture; int btn, lx, ly; HWND hWnd; } ix;
...
void Interaction_BeginEx(HWND hWnd, int x, int y, int btn, int tool, BOOL capture) {
    (void)tool;
    Layers_BeginWrite();
    ix.active = TRUE; ix.modified = FALSE; ix.btn = btn;
...
BOOL Interaction_Commit(const char *label) {
    ...
    if (ix.modified || draft) HistoryPush(label ? label : "Draw");
...
void Interaction_Abort(void) {
    if (!ix.active) return;
    LayersClearDraft(); EndState(); InvalidateRect(GetCanvasWindow(), NULL, FALSE); Core_Notify(EV_PIXELS_CHANGED);
}
...
void Interaction_OnCaptureLost(const char *label) {
    if (!ix.active) return;
    if (ix.modified) HistoryPush(label ? label : "Draw");
    else Core_Notify(EV_PIXELS_CHANGED);
    EndState(); SetDocumentDirty(); InvalidateRect(GetCanvasWindow(), NULL, FALSE);
}
