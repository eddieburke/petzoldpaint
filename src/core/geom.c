...
Viewport GetCurrentViewport(void) {
	Viewport vp;
	vp.scale = GetZoomScale();
	GetCanvasViewportOrigin(&vp.offX, &vp.offY);
	return vp;
}
void Viewport_BmpToScr(const Viewport *vp, int xBmp, int yBmp, int *xScr, int *yScr) {
	*xScr = (int)floor((double)xBmp * vp->scale) + vp->offX;
	*yScr = (int)floor((double)yBmp * vp->scale) + vp->offY;
}
void Viewport_RectBmpToScr(const Viewport *vp, const RECT *rcBmp, RECT *rcScr) {
	rcScr->left = (LONG)((int)floor(rcBmp->left * vp->scale) + vp->offX);
	rcScr->top = (LONG)((int)floor(rcBmp->top * vp->scale) + vp->offY);
	rcScr->right = (LONG)((int)ceil(rcBmp->right * vp->scale) + vp->offX);
	rcScr->bottom = (LONG)((int)ceil(rcBmp->bottom * vp->scale) + vp->offY);
	if (rcBmp->right > rcBmp->left && rcScr->right <= rcScr->left)
		rcScr->right = rcScr->left + 1;
	if (rcBmp->bottom > rcBmp->top && rcScr->bottom <= rcScr->top)
		rcScr->bottom = rcScr->top + 1;
}
void Viewport_ScrToBmpDouble(const Viewport *vp, int xScr, int yScr, double *xBmp, double *yBmp) {
	if (xBmp)
		*xBmp = ((double)xScr + 0.5 - vp->offX) / vp->scale;
	if (yBmp)
		*yBmp = ((double)yScr + 0.5 - vp->offY) / vp->scale;
}
void Viewport_ScrToBmp(const Viewport *vp, int xScr, int yScr, int *xBmp, int *yBmp) {
	double fx, fy;
	Viewport_ScrToBmpDouble(vp, xScr, yScr, &fx, &fy);
	if (xBmp)
		*xBmp = (int)floor(fx);
	if (yBmp)
		*yBmp = (int)floor(fy);
...
int HitTestBoxHandles(const RECT *rc, int x, int y, int tolerance) {
	int xm = rc->left + ((rc->right - rc->left) / 2);
	int ym = rc->top + ((rc->bottom - rc->top) / 2);
	int r = tolerance;
	POINT pts[8] = {{rc->left, rc->top}, {xm, rc->top}, {rc->right, rc->top}, {rc->right, ym}, {rc->right, rc->bottom}, {xm, rc->bottom}, {rc->left, rc->bottom}, {rc->left, ym}};
	for (int i = 0; i < 8; i++) {
		if (abs(x - pts[i].x) <= r && abs(y - pts[i].y) <= r)
			return i;
	}
