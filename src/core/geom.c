...
{
    if (xBmp) *xBmp = ((double)xScr + 0.5 - vp->offX) / vp->scale;
    if (yBmp) *yBmp = ((double)yScr + 0.5 - vp->offY) / vp->scale;
}
...
int HitTestBoxHandles(const RECT* rc, int x, int y, int tolerance)
{
    ...
    POINT pts[8] = {
        {rc->left, rc->top},     {xm, rc->top},
        {rc->right, rc->top},    {rc->right, ym},
        {rc->right, rc->bottom}, {xm, rc->bottom},
        {rc->left, rc->bottom},  {rc->left, ym}
    };
