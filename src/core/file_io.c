...
static BOOL EnsureWicFactory(void) {
	...
	if (!g_wicFactory) {
		HRESULT hr = CoCreateInstance(&CLSID_WICImagingFactory, NULL, CLSCTX_INPROC_SERVER, &IID_IWICImagingFactory, (LPVOID *)&g_wicFactory);
		if (FAILED(hr))
			return FALSE;
	}
	return TRUE;
...
static BOOL SaveWicImage(const wchar_t *lpszFileName, REFGUID containerFormat) {
	...
	if (lpszFileName && lpszFileName[0] != L'\0') {
		...
		if (SUCCEEDED(hr)) {
			hr = stream->lpVtbl->InitializeFromFilename(stream, lpszFileName, GENERIC_WRITE);
		}
		if (SUCCEEDED(hr)) {
			hr = g_wicFactory->lpVtbl->CreateEncoder(g_wicFactory, containerFormat, NULL, &encoder);
		}
		if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr)) {
			...
			hr = g_wicFactory->lpVtbl->CreateBitmapFromMemory(g_wicFactory, Canvas_GetWidth(), Canvas_GetHeight(), &GUID_WICPixelFormat32bppBGRA, Canvas_GetWidth() * 4, Canvas_GetWidth() * Canvas_GetHeight() * 4, bits, &wicBitmap);
			if (SUCCEEDED(hr)) {
				IWICFormatConverter *converter = NULL;
				hr = g_wicFactory->lpVtbl->CreateFormatConverter(g_wicFactory, &converter);
				if (SUCCEEDED(hr)) {
					hr = converter->lpVtbl->Initialize(converter, (IWICBitmapSource *)wicBitmap, &format, WICBitmapDitherTypeNone, NULL, 0.0, WICBitmapPaletteTypeCustom);
					if (SUCCEEDED(hr)) {
...
BOOL LoadBitmapFromFile(const wchar_t *szPath) {
	if (HasExtension(szPath, L".png") || HasExtension(szPath, L".jpg") || HasExtension(szPath, L".jpeg") || HasExtension(szPath, L".gif") || HasExtension(szPath, L".tif") || HasExtension(szPath, L".tiff")) {
		return LoadWicImage(szPath);
	}
	HBITMAP hNewBmp = (HBITMAP)LoadImageW(NULL, szPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	if (!hNewBmp)
		return FALSE;
...
BOOL FileSaveAs(HWND hWnd) {
	...
	if (GetSaveFileNameW(&ofn)) {
		if (!HasExtension(ofn.lpstrFile, L".png") && !HasExtension(ofn.lpstrFile, L".bmp") && !HasExtension(ofn.lpstrFile, L".jpg") && !HasExtension(ofn.lpstrFile, L".jpeg")) {
