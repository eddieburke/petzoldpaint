...
static void CommitBar_GetRects(const RECT *targetBmpRect, RECT *outCommitScr, RECT *outCancelScr) {
	...
	CoordBmpToScr(targetBmpRect->right, targetBmpRect->bottom, &rightScr, &bottomScr);
	if (rightScr < leftScr) {
		int t = leftScr;
		leftScr = rightScr;
		rightScr = t;
	...
	SetRect(outCancelScr, barX, barY + btnSize + gap, barX + btnSize, barY + btnSize + gap + btnSize);
}
BOOL CommitBar_IsEnabled(void) {
	return s_commitBarEnabled;
}
...
void CommitBar_Draw(const OverlayContext *ctx, const RECT *targetBmpRect) {
	...
	HPEN hOldPen = NULL;
	HBRUSH hCommitBr = CreateBrushAndSelect(ctx->hdc, RGB(228, 248, 228), &hOldBrush);
	HPEN hCommitPen = CreatePenAndSelect(ctx->hdc, PS_SOLID, 1, RGB(32, 128, 32), &hOldPen);
	Rectangle(ctx->hdc, rcCommit.left, rcCommit.top, rcCommit.right, rcCommit.bottom);
	...
	HBRUSH hCancelBr = CreateBrushAndSelect(ctx->hdc, RGB(252, 230, 230), &hOldBrush);
	HPEN hCancelPen = CreatePenAndSelect(ctx->hdc, PS_SOLID, 1, RGB(176, 48, 48), &hOldPen);
	Rectangle(ctx->hdc, rcCancel.left, rcCancel.top, rcCancel.right, rcCancel.bottom);
...
int CommitBar_HitTestScreen(const RECT *targetBmpRect, int xScr, int yScr) {
	if (!targetBmpRect || !s_commitBarEnabled)
		return COMMIT_BAR_HIT_NONE;
	RECT rcCommit, rcCancel;
	CommitBar_GetRects(targetBmpRect, &rcCommit, &rcCancel);
