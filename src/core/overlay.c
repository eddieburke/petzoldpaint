...
void Overlay_DrawHandle(const OverlayContext *ctx, int xBmp, int yBmp, int type, BOOL bHollow) {
	...
	HBRUSH hBr = bHollow ? (HBRUSH)GetStockObject(NULL_BRUSH) : (HBRUSH)GetStockObject(WHITE_BRUSH);
...
void Overlay_DrawBoxHandles(const OverlayContext *ctx, const RECT *rcBitmap) {
	...
	Overlay_DrawHandle(ctx, rcBitmap->left, rcBitmap->top, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, xm, rcBitmap->top, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, rcBitmap->right, rcBitmap->top, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, rcBitmap->right, ym, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, rcBitmap->right, rcBitmap->bottom, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, xm, rcBitmap->bottom, OVERLAY_HANDLE_SQUARE, FALSE);
	Overlay_DrawHandle(ctx, rcBitmap->left, rcBitmap->bottom, OVERLAY_HANDLE_SQUARE, FALSE);
...
void Overlay_DrawPolyHandles(const OverlayContext *ctx, const POINT *ptsBitmap, int count) {
	for (int i = 0; i < count; i++) {
		Overlay_DrawHandle(ctx, ptsBitmap[i].x, ptsBitmap[i].y, OVERLAY_HANDLE_CIRCLE, FALSE);
	}
}
...
void Overlay_DrawLine(const OverlayContext *ctx, int x1Bmp, int y1Bmp, int x2Bmp, int y2Bmp, COLORREF color, int penStyle) {
...
void Overlay_DrawPolyline(const OverlayContext *ctx, const POINT *ptsBitmap, int count, COLORREF color, int penStyle) {
	if (count < 2)
		return;
	...
	if (!ptsScreen)
		return;
	for (int i = 0; i < count; i++) {
		CoordBmpToScr(ptsBitmap[i].x, ptsBitmap[i].y, &ptsScreen[i].x, &ptsScreen[i].y);
	}
	HPEN hOldPen = NULL;
	HPEN hPen = CreatePenAndSelect(ctx->hdc, penStyle, 1, color, &hOldPen);
	if (hPen) {
