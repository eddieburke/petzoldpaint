...
void PixelOps_Fill(BYTE *bits, int width, int height, COLORREF color, BYTE alpha) {
	if (!bits || width <= 0 || height <= 0)
		return;
...
void PixelOps_FillRect(BYTE *bits, int width, int height, COLORREF color, BYTE alpha, int startX, int startY, int endX, int endY) {
	if (!bits || width <= 0 || height <= 0)
		return;
	if (startX < 0)
		startX = 0;
...
void PixelOps_FillCheckerboardRect(BYTE *bits, int width, int height, int startX, int startY, int endX, int endY) {
	if (!bits || width <= 0 || height <= 0)
		return;
	// Clamp rect
	if (startX < 0)
	...
	int tile = 8;
	DWORD d1 = (DWORD)(GetBValue(c1) | (GetGValue(c1) << 8) | (GetRValue(c1) << 16) | (255 << 24));
	DWORD d2 = (DWORD)(GetBValue(c2) | (GetGValue(c2) << 8) | (GetRValue(c2) << 16) | (255 << 24));
	for (int y = startY; y < endY; y++) {
		int yTile = (y / tile) & 1;
		DWORD *row = (DWORD *)(bits + y * width * 4);
		for (int x = startX; x < endX; x++) {
