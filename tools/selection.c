/*------------------------------------------------------------------------------
 * SELECTION.C - Simplified Selection Subsystem
 *----------------------------------------------------------------------------*/
...
static void SampleRotated(BYTE *src, int sw, int sh, BYTE *dst, double angle, double cx, double cy, int sx, int sy, int ex, int ey, int cw, int ch);
...
void LiftSelectionPixels(void) {
    if (s_sel.mode != SEL_REGION_ONLY) return;
    int w = s_sel.rcBounds.right - s_sel.rcBounds.left, h = s_sel.rcBounds.bottom - s_sel.rcBounds.top;
    if (w <= 0 || h <= 0) return;
    BackupCanvas();
    s_sel.hFloatBmp = CreateDibSection32(w, h, &s_sel.pFloatBits);
    if (!s_sel.hFloatBmp) return;
    s_sel.nFloatW = w; s_sel.nFloatH = h;
    memset(s_sel.pFloatBits, 0, (size_t)w * h * 4);
    HBITMAP hOld; HDC hDC = GetCanvasBitmapDC(&hOld);
    if (hDC) {
        ...
        SelectObject(fDC, hOldF); DeleteDC(fDC);
    ...
    s_sel.fAngle = 0; s_sel.mode = SEL_FLOATING;
    UpdateDraft(); UpdateCanvasAfterModification();
}

static void RestoreCanvas(void) {
    if (!s_sel.hBackupBmp || !s_sel.pBackupBits) return;
    BYTE *cb = LayersGetActiveColorBits(); if (!cb) return;
...
static void GetRotatedExtents(int *dw, int *dh, double *cx, double *cy, int *sx, int *sy, int *ex, int *ey) {
    int w = s_sel.rcBounds.right - s_sel.rcBounds.left, h = s_sel.rcBounds.bottom - s_sel.rcBounds.top;
    *dw = w; *dh = h;
    double cxLocal = s_sel.rcBounds.left + w / 2.0, cyLocal = s_sel.rcBounds.top + h / 2.0;
    if (cx) *cx = cxLocal; if (cy) *cy = cyLocal;
    if (w <= 0 || h <= 0) { if(sx)*sx=0; if(sy)*sy=0; if(ex)*ex=0; if(ey)*ey=0; return; }
    double rad = s_sel.fAngle * M_PI / 180.0, c = cos(rad), s = sin(rad), hw = w/2.0, hh = h/2.0;
    double pts[4][2] = {{-hw,-hh},{hw,-hh},{hw,hh},{-hw,hh}}, mnX=0,mxX=0,mnY=0,mxY=0;
    for (int i=0;i<4;i++) { double rx=pts[i][0]*c-pts[i][1]*s, ry=pts[i][0]*s+pts[i][1]*c;
        if(i==0||rx<mnX)mnX=rx; if(i==0||rx>mxX)mxX=rx; if(i==0||ry<mnY)mnY=ry; if(i==0||ry>mxY)mxY=ry; }
    if(sx)*sx=(int)floor(cxLocal+mnX); if(ex)*ex=(int)ceil(cxLocal+mxX); if(sy)*sy=(int)floor(cyLocal+mnY); if(ey)*ey=(int)ceil(cyLocal+mxY);
}
...
void CommitSelection(void) {
    ...
    if (s_sel.mode == SEL_FLOATING && s_sel.pFloatBits) {
        ...
        if (bits) {
            int dw,dh,sx,sy,ex,ey; double cx,cy;
            GetRotatedExtents(&dw,&dh,&cx,&cy,&sx,&sy,&ex,&ey);
            if (dw > 0 && dh > 0) SampleRotated(s_sel.pFloatBits, s_sel.nFloatW, s_sel.nFloatH, bits, s_sel.fAngle, cx, cy, sx, sy, ex, ey, Canvas_GetWidth(), Canvas_GetHeight());
        }
        UpdateCanvasAfterModification(); SetDocumentDirty(); SelectionClearState();
        HistoryPushToolActionById(TOOL_SELECT, "Commit Selection");
    } else { SelectionClearState(); InvalidateCanvas(); }
}
...
void CancelSelection(void) {
    ...
    if (s_sel.mode == SEL_FLOATING && s_sel.hBackupBmp) {
        RestoreCanvas();
        HRGN old = s_sel.hRegion; s_sel.hRegion = s_sel.hBackupRegion; s_sel.hBackupRegion = NULL;
        if (old) DeleteObject(old);
        s_sel.rcBounds = s_sel.rcLiftOrigin; s_sel.mode = SEL_REGION_ONLY;
        s_sel.fAngle = 0;
        if (s_sel.hFloatBmp) { DeleteObject(s_sel.hFloatBmp); s_sel.hFloatBmp = NULL; s_sel.pFloatBits = NULL; }
        if (s_sel.hBackupBmp) { DeleteObject(s_sel.hBackupBmp); s_sel.hBackupBmp = NULL; s_sel.pBackupBits = NULL; }
        LayersClearDraft(); UpdateCanvasAfterModification();
    } else { SelectionClearState(); LayersClearDraft(); InvalidateCanvas(); }
}

SelectionSnapshot* Selection_CreateSnapshot(void) {
    SelectionSnapshot* s = calloc(1, sizeof(SelectionSnapshot));
    if (!s) return NULL;
    s->mode = s_sel.mode; s->rcBounds = s_sel.rcBounds;
    if (s_sel.hRegion) { s->hRegion = CreateRectRgn(0,0,0,0); CombineRgn(s->hRegion, s_sel.hRegion, NULL, RGN_COPY); }
    if (s_sel.hFloatBmp) { s->hFloatBmp = CopyBitmapToDib32(s_sel.hFloatBmp, s_sel.nFloatW, s_sel.nFloatH, &s->pFloatBits); s->nFloatW = s_sel.nFloatW; s->nFloatH = s_sel.nFloatH; }
    if (s_sel.hBackupBmp) { s->hBackupBmp = CopyBitmapToDib32(s_sel.hBackupBmp, Canvas_GetWidth(), Canvas_GetHeight(), &s->pBackupBits); }
    if (s_sel.hBackupRegion) { s->hBackupRegion = CreateRectRgn(0,0,0,0); CombineRgn(s->hBackupRegion, s_sel.hBackupRegion, NULL, RGN_COPY); }
    s->rcLiftOrigin = s_sel.rcLiftOrigin; s->fRotationAngle = s_sel.fAngle;
    s->ptRotateCenter = s_sel.ptCenter; Poly_Copy(&s->freeformPts, &s_sel.freeformPts);
    return s;
}
...
void Selection_DestroySnapshot(SelectionSnapshot* s) {
    ...
    Poly_Free(&s->freeformPts); free(s);
}

void Selection_ApplySnapshot(SelectionSnapshot* s) {
    SelectionClearState();
    if (!s) { InvalidateCanvas(); return; }
    s_sel.mode = s->mode; s_sel.rcBounds = s->rcBounds;
    if (s->hRegion) { s_sel.hRegion = CreateRectRgn(0,0,0,0); CombineRgn(s_sel.hRegion, s->hRegion, NULL, RGN_COPY); }
    if (s->hFloatBmp) { s_sel.hFloatBmp = CopyBitmapToDib32(s->hFloatBmp, s->nFloatW, s->nFloatH, &s_sel.pFloatBits); s_sel.nFloatW = s->nFloatW; s_sel.nFloatH = s->nFloatH; }
    if (s->hBackupBmp) { s_sel.hBackupBmp = CopyBitmapToDib32(s->hBackupBmp, Canvas_GetWidth(), Canvas_GetHeight(), &s_sel.pBackupBits); }
    if (s->hBackupRegion) { s_sel.hBackupRegion = CreateRectRgn(0,0,0,0); CombineRgn(s_sel.hBackupRegion, s->hBackupRegion, NULL, RGN_COPY); }
    s_sel.rcLiftOrigin = s->rcLiftOrigin; s_sel.fAngle = s->fRotationAngle;
    s_sel.ptCenter = s->ptRotateCenter; Poly_Copy(&s_sel.freeformPts, &s->freeformPts);
    s_sel.nDragMode = HT_NONE;
    if (s_sel.mode == SEL_FLOATING && !s_sel.pFloatBits) s_sel.mode = SEL_REGION_ONLY;
    UpdateDraft(); InvalidateCanvas();
}
...
void SelectionFlip(BOOL horz) {
    ...
    UpdateDraft(); HistoryPushToolSessionById(TOOL_SELECT, "Adjust Selection");
}

void SelectionRotate(int deg) {
    if (s_sel.mode != SEL_FLOATING) LiftSelectionPixels();
    if (s_sel.mode != SEL_FLOATING) return;
    s_sel.fAngle += deg; UpdateDraft();
    HistoryPushToolSessionById(TOOL_SELECT, "Adjust Selection");
}

static void UpdateDraft(void) {
    if (s_sel.mode != SEL_FLOATING || !s_sel.pFloatBits) { LayersClearDraft(); return; }
    BYTE *draft = LayersGetDraftBits(); if (!draft) return;
    LayersClearDraft();
    int dw,dh,sx,sy,ex,ey; double cx,cy;
    GetRotatedExtents(&dw,&dh,&cx,&cy,&sx,&sy,&ex,&ey);
    if (dw > 0 && dh > 0) SampleRotated(s_sel.pFloatBits, s_sel.nFloatW, s_sel.nFloatH, draft, s_sel.fAngle, cx, cy, sx, sy, ex, ey, Canvas_GetWidth(), Canvas_GetHeight());
    LayersMarkDirty();
}

static void BlitFloatToCanvas(void) {
    if (s_sel.mode != SEL_FLOATING || !s_sel.pFloatBits) return;
    Layers_BeginWrite();
    BYTE *bits = LayersGetActiveColorBits(); if (!bits) return;
    int dw,dh,sx,sy,ex,ey; double cx,cy;
    GetRotatedExtents(&dw,&dh,&cx,&cy,&sx,&sy,&ex,&ey);
    if (dw > 0 && dh > 0) SampleRotated(s_sel.pFloatBits, s_sel.nFloatW, s_sel.nFloatH, bits, s_sel.fAngle, cx, cy, sx, sy, ex, ey, Canvas_GetWidth(), Canvas_GetHeight());
}
...
static HGLOBAL ExportPNG(void) {
    ...
    IStream *stream = NULL; IWICBitmapEncoder *enc = NULL; IWICBitmapFrameEncode *frame = NULL;
    HGLOBAL hMem = NULL;
...
static HGLOBAL ExportDIBV5(void) {
    ...
    v5->bV5Size = sizeof(BITMAPV5HEADER); v5->bV5Width = w; v5->bV5Height = -h;
    v5->bV5Planes = 1; v5->bV5BitCount = 32; v5->bV5Compression = BI_BITFIELDS;
    v5->bV5RedMask = 0x00FF0000; v5->bV5GreenMask = 0x0000FF00; v5->bV5BlueMask = 0x000000FF;
    v5->bV5AlphaMask = 0xFF000000; v5->bV5CSType = LCS_sRGB; v5->bV5Intent = LCS_GM_IMAGES;
...
void SelectionCopy(void) {
    ...
    if (OpenClipboard(NULL)) {
        EmptyClipboard();
        HGLOBAL hPng = ExportPNG(); if (hPng) SetClipboardData(RegisterClipboardFormat("PNG"), hPng);
        HGLOBAL hDib = ExportDIBV5(); if (hDib) SetClipboardData(CF_DIBV5, hDib);
        CloseClipboard();
    }
}

void SelectionCut(void) {
    SelectionCopy(); DeleteInternal(FALSE, "Cut");
    if (!HistoryPush("Cut")) {}
}
...
void SelectionPaste(HWND hWnd) {
    ...
    HGLOBAL hDat = NULL; int w = 0, h = 0;
    UINT uPng = RegisterClipboardFormat("PNG");
    IWICImagingFactory *f = FileIO_GetWicFactory();
    if ((hDat = GetClipboardData(uPng)) && f) {
        IStream *stream = NULL; IWICBitmapDecoder *dec = NULL; IWICBitmapFrameDecode *fr = NULL; IWICFormatConverter *conv = NULL;
                ...
                if (SUCCEEDED(dec->lpVtbl->GetFrame(dec, 0, &fr))) {
                    if (SUCCEEDED(f->lpVtbl->CreateFormatConverter(f, &conv))) {
                        if (SUCCEEDED(conv->lpVtbl->Initialize(conv, (IWICBitmapSource*)fr, &GUID_WICPixelFormat32bppBGRA, WICBitmapDitherTypeNone, NULL, 0.0, WICBitmapPaletteTypeCustom))) {
                            UINT uw, uh; conv->lpVtbl->GetSize(conv, &uw, &uh);
                            s_sel.hFloatBmp = CreateDibSection32(uw, uh, &s_sel.pFloatBits);
                            if (s_sel.hFloatBmp) { conv->lpVtbl->CopyPixels(conv, NULL, uw * 4, uw * uh * 4, s_sel.pFloatBits); w = uw; h = uh; }
                        }
                    }
                }
            }
        }
        if (conv) conv->lpVtbl->Release(conv); if (fr) fr->lpVtbl->Release(fr);
        if (dec) dec->lpVtbl->Release(dec); if (stream) stream->lpVtbl->Release(stream);
        ...
        if (p) {
            ...
            if (v5->bV5BitCount == 32 && v5->bV5SizeImage == (DWORD)(w * h * 4)) {
                ...
                if (s_sel.hFloatBmp) {
                    ...
                    if (v5->bV5AlphaMask == 0) for (int i = 0; i < w * h; i++) s_sel.pFloatBits[i * 4 + 3] = 255;
                }
            }
            GlobalUnlock(hDat);
        }
    ...
    CloseClipboard();
    if (s_sel.hFloatBmp) {
        s_sel.nFloatW = w; s_sel.nFloatH = h; s_sel.mode = SEL_FLOATING;
        s_sel.rcBounds = (RECT){0, 0, w, h}; UpdateDraft(); InvalidateCanvas();
    }
}
...
static void DeleteInternal(BOOL pushHist, const char *label) {
    ...
    if (s_sel.mode == SEL_FLOATING) {
        ...
        SelectionClearState(); LayersClearDraft(); UpdateCanvasAfterModification();
        if (pushHist && hadBackup && !HistoryPush(lbl)) {}
    } else if (s_sel.mode == SEL_REGION_ONLY) {
        Layers_BeginWrite();
        BYTE *p = LayersGetActiveColorBits(); int cw = Canvas_GetWidth(), ch = Canvas_GetHeight();
        for (int y = s_sel.rcBounds.top; y < s_sel.rcBounds.bottom; y++) for (int x = s_sel.rcBounds.left; x < s_sel.rcBounds.right; x++)
            if (x >= 0 && y >= 0 && x < cw && y < ch && (!s_sel.hRegion || PtInRegion(s_sel.hRegion, x, y)))
                *(DWORD*)(p + (y * cw + x) * 4) = 0;
        UpdateCanvasAfterModification(); SelectionClearState();
        if (pushHist && !HistoryPush(lbl)) {}
    }
}
...
static int HitTestRotationHandles(RECT *rc, int x, int y) {
    double scale = GetZoomScale(); int tol = max(1, (int)(6.0 / scale)), rotTol = (int)(20.0 / scale);
    int L = rc->left, T = rc->top, R = rc->right > rc->left ? rc->right - 1 : L, B = rc->bottom > rc->top ? rc->bottom - 1 : T;
    POINT c[4] = {{L,T},{R,T},{R,B},{L,B}}; int h[4] = {HT_ROTATE_TL, HT_ROTATE_TR, HT_ROTATE_BR, HT_ROTATE_BL};
    for (int i = 0; i < 4; i++) { int dx = x - c[i].x, dy = y - c[i].y, d2 = dx*dx + dy*dy;
...
void SelectionToolOnMouseDown(HWND hWnd, int x, int y, int nButton) {
    ...
    s_modeAtDragStart = s_sel.mode; s_dragMoved = FALSE; s_toolAtDragStart = Tool_GetCurrent();
    if (s_sel.mode != SEL_NONE) {
        RECT rcBar; GetRotatedExtents(&rcBar.right, &rcBar.bottom, NULL, NULL, &rcBar.left, &rcBar.top, &rcBar.right, &rcBar.bottom);
        COMMIT_BAR_HANDLE_CLICK(&rcBar, x, y, CommitSelection(), CancelSelection());
        RECT rcFrame; GetRotatedExtents(&rcFrame.right, &rcFrame.bottom, NULL, NULL, &rcFrame.left, &rcFrame.top, &rcFrame.right, &rcFrame.bottom);
    ...
    s_sel.nDragMode = HT_BODY; s_sel.ptDragStart = (POINT){x, y}; s_sel.rcBounds = (RECT){x, y, x, y};
    if (s_toolAtDragStart == TOOL_FREEFORM) { Poly_Clear(&s_sel.freeformPts); Poly_Add(&s_sel.freeformPts, x, y); }
    SetCapture(hWnd);
}
...
void SelectionToolOnMouseMove(HWND hWnd, int x, int y, int nButton) {
    ...
    if (s_sel.nDragMode >= HT_ROTATE_TL && s_sel.nDragMode <= HT_ROTATE_BL) {
        ...
        if (s_toolAtDragStart == TOOL_FREEFORM) {
            Poly_Add(&s_sel.freeformPts, x, y); s_sel.rcBounds = Poly_GetBounds(&s_sel.freeformPts);
        } else {
        ...
        s_dragMoved = TRUE; s_sel.rcBounds = s_sel.rcDragOrig;
...
void SelectionToolOnMouseUp(HWND hWnd, int x, int y, int nButton) {
    ...
    if (s_modeAtDragStart == SEL_NONE && !s_dragMoved) { SelectionClearState(); LayersClearDraft(); InvalidateCanvas(); }
    else if (s_toolAtDragStart == TOOL_FREEFORM && s_sel.freeformPts.count > 2) s_sel.hRegion = Poly_CreateRegion(&s_sel.freeformPts);
    if (s_dragMoved && s_sel.mode != SEL_NONE) HistoryPushToolSessionById(TOOL_SELECT, s_modeAtDragStart == SEL_NONE ? "Create Selection" : "Adjust Selection");
    s_sel.nDragMode = HT_NONE; s_dragMoved = FALSE; s_toolAtDragStart = TOOL_SELECT;
}

void SelectionTool_OnCaptureLost(void) {
    s_sel.nDragMode = HT_NONE; s_dragMoved = FALSE;
    if (s_modeAtDragStart == SEL_NONE && s_sel.mode == SEL_REGION_ONLY) { SelectionClearState(); LayersClearDraft(); InvalidateCanvas(); }
}
...
BOOL IsPointInSelection(int x, int y) {
    ...
    if (fabs(s_sel.fAngle) > 0.01) {
        ...
        fx = cx + dx * c - dy * s; fy = cy + dx * s + dy * c;
    }
...
void SelectionInvert(void) {
    ...
    if (s_sel.mode == SEL_NONE) {
        SelectionSelectAll();
        HistoryPushToolSessionById(TOOL_SELECT, "Invert Selection");
        return;
    }
...
void SelectionInvertColors(void) {
    ...
    for (int i = 0; i < s_sel.nFloatW * s_sel.nFloatH; i++) {
        BYTE *p = s_sel.pFloatBits + i * 4; p[0] = 255 - p[0]; p[1] = 255 - p[1]; p[2] = 255 - p[2];
    }
    UpdateDraft();
}

void SelectionSelectAll(void) {
    SelectionClearState(); s_sel.rcBounds = (RECT){0, 0, Canvas_GetWidth(), Canvas_GetHeight()};
    s_sel.mode = SEL_REGION_ONLY; InvalidateCanvas();
}

void SelectionMove(int dx, int dy) {
    if (s_sel.mode != SEL_FLOATING) LiftSelectionPixels();
    if (s_sel.mode == SEL_FLOATING) { OffsetRect(&s_sel.rcBounds, dx, dy); UpdateDraft(); InvalidateCanvas();
        HistoryPushToolSessionById(TOOL_SELECT, "Adjust Selection"); }
}