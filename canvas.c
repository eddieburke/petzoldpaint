...
// Old undo system replaced with history system
...
#define HANDLE_SZ 6
  ...
  Rectangle(hdc, nDestX + nScaledW + 2, nDestY + nScaledH + 2,
            nDestX + nScaledW + 2 + HANDLE_SZ,
            nDestY + nScaledH + 2 + HANDLE_SZ);

  Rectangle(hdc, nDestX + nScaledW + 2, nDestY + nScaledH / 2 - HANDLE_SZ / 2,
            nDestX + nScaledW + 2 + HANDLE_SZ,
            nDestY + nScaledH / 2 + HANDLE_SZ / 2);

  Rectangle(hdc, nDestX + nScaledW / 2 - HANDLE_SZ / 2, nDestY + nScaledH + 2,
            nDestX + nScaledW / 2 + HANDLE_SZ / 2,
            nDestY + nScaledH + 2 + HANDLE_SZ);
...
void RefreshCanvasRect(RECT *pRect) {
  if (hCanvasWnd) {
    InvalidateRect(hCanvasWnd, pRect, FALSE);
